...
server <- function(input, output, session) {
  ...
  observe({
    dna <- toupper(gsub("[^ATCG]", "", input$dna_input))
    rna <- toupper(gsub("[^AUCG]", "", input$rna_input))
    updateTextInput(session, "dna_input", value = dna)
    updateTextInput(session, "rna_input", value = rna)
  })
  
  # Validate inputs and update border colors
  observe({
    dna_valid <- validate_dna(input$dna_input) == input$dna_input
    rna_valid <- if (nchar(input$rna_input) > 0) {
      validate_rna(input$dna_input, input$rna_input) == input$rna_input
    } else {
      TRUE
    }
